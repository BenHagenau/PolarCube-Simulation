...
if plots.omega == 1
    ...
    title('\omega, true angular velocity')
    xlabel('Simulation Time [s]')
...
if plots.HSV == 1
    ...
    title('hSV, satellite bus angular momentum')
    drawnow
end
...
if plots.HRW == 1
    ...
    title('hVSCMG, reaction wheel angular momentum')
    drawnow
end
...
if plots.HM == 1
    ...
    title('hM, mirror angular momentum')
    drawnow
end
...
if plots.T == 1
    ...
    title('System Kinetic Energy Variation')
    xlabel('Simulation Time [s]')
...
if plots.us == 1
    ...
    title('u_s, reaction wheel torques')
...
if plots.OmegaDot == 1
    ...
    title('OmegaDot, wheel acceleration')
    xlabel('Simulation Time [s]')
...
if plots.mu == 1
    ...
    title('\mu, mirror angular position')
    xlabel('Simulation Time [s]')
...
if plots.muDot == 1
    ...
    title('\mu dot, mirror rotation rate')
    xlabel('Simulation Time [s]')
...
if plots.Phi == 1
    ...
    title('\Phi, principle rotation angle error')
    xlabel('Simulation Time [s]')
...
if plots.pointErr == 1
    ...
    semilogy(simResults.t(psk:end),simResults.pointReq(psk:end),'.r',...
        'Linewidth',2)
    title('\Phi, log-scale pointing error')
    xlabel('Simulation Time [s]')
    ylabel('Attitude Error [rad]')
    xlim([0 max(simResults.t)])
    drawnow
...
if busStopInd == 0 && ~isempty(busStartInd)
...
title('Solar Panel Incidence Angle')
xlabel('Simulation Time [s]')
